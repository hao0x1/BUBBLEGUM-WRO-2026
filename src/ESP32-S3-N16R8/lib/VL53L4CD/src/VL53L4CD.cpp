...
bool VL53L4CD::init(bool io_2v8, bool fast_mode_plus)
{
  ...
  if (!setRangeTiming(50, 0)) { return false; }
...
uint8_t VL53L4CD::readReg(uint16_t reg)
{
  ...
  bus->write((uint8_t)(reg));      // reg low byte
  last_status = bus->endTransmission();
  if (last_status != 0) { return 0xFF; }
  if (bus->requestFrom(address, (uint8_t)1) != 1)
  {
    last_status = 4; // Wire "other error"; never consume a missing byte as data.
...
uint16_t VL53L4CD::readReg16Bit(uint16_t reg)
{
  ...
  uint16_t value = (uint16_t)bus->read() << 8; // value high byte
...
uint32_t VL53L4CD::readReg32Bit(uint16_t reg)
{
  ...
  uint32_t value = (uint32_t)bus->read() << 24; // value highest byte
...
bool VL53L4CD::setRangeTiming(uint8_t timing_budget_ms, uint32_t inter_measurement_ms)
{
  ...
  if (last_status != 0 || osc_frequency == 0)
  {
    return false;
  }
  ...
  else
  {
    ...
    if (last_status != 0 || clock_pll == 0) { return false; }
...
{
  uint16_t osc_frequency = readReg16Bit(0x06);
  if (last_status != 0 || osc_frequency == 0) { return 0; }
  ...
  const uint32_t inter_measurement = readReg32Bit(INTERMEASUREMENT_MS);
  if (last_status != 0) { return 0; }
...
uint32_t VL53L4CD::getInterMeasurement()
{
  ...
  if (last_status != 0 || clock_pll == 0) { return 0; }
...
bool VL53L4CD::readResults()
{
  // Never leave a previous valid sample visible after a failed transaction.
  ranging_data = {};
  ranging_data.range_status = 255;
  ...
  last_status = bus->endTransmission();
  if (last_status != 0) { return false; }
  if (bus->requestFrom(address, (uint8_t)15) != 15)
  {
    last_status = 4;
    while (bus->available()) { bus->read(); }
    return false;
  ...
  {
    // A malformed/partial result must be invalid data, never divide-by-zero.
    ranging_data.range_status = 255;
